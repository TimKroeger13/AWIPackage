#'MaxKorrelation
#'@description Calculates all Max and Min values for an Folder structure.
#'@export
...
MaxKorrelation = function(main,folder,thick,offset,IgnorePercent=0.1){
  ...
  for(i in 1:length(ls(folder))){
    ...
    for(k in 1:length(ls(folder[[i]]))){
      ...
      if((sum(folder[[i]][[k]]>0,na.rm = T))>(length(folder[[i]][[k]])*IgnorePercent)){

        TempUnderfolder[[TempNames[k]]]=folder[[i]][[k]]
